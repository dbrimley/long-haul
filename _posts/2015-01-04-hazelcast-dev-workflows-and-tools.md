---
layout: post
title: "Hazelcast Dev Workflows & Tools"
modified:
categories: blog
excerpt: I needed a way to quickly switch between different Hazelcast development environments that I had for my customers. I'd heard of tools like Vagrant & Chef but never really had the opportunity to use them.
tags: [Vagrant, Chef, Hazelcast]
image: hazelcast-dev-tools-workflow.png
date: 2015-01-04T20:47:25+00:00
comments: true
---
It all began when I started working for Hazelcast a year ago this month. In previous roles I'd always worked on one project at a time and usually had a dedicated set of UAT and test servers that I could deploy to, along with a Continuous Integration system.

This all changed with my new role, where I was working from home a lot of the time and I had quite a few customer projects running at one time. I had to switch between different development and deployment platforms on a regular basis, all on my own laptop. I needed a way to package up code, deliver it and demonstrate it on various platforms that could scale out easily. I had to use the best of what I could find and if possible for free and open source.

What I want to share with you are my go to tools for a desktop based Hazelcast Development environment.

##What this isn't
This isn't an exhaustive guide to the tools in question, you can find out a lot more if you're a beginner by following the links I'll provide later on.  But, what you should get out of this post is get up and running with a Hazelcast Development environment that you can run on virtual machines, on your desktop, and then easily scale it out on more powerful hardware such as Amazon EC2.

##What this is...

1. A bare-bones starter platform for somebody wishing to develop Hazelcast systems and deploy them onto multiple different platforms at different scales.

2. An example of deploying a Hazelcast project onto Amazon EC2 using third party tools such as [Vagrant](https://www.vagrantup.com/) and [Chef](https://www.chef.io/chef/)

3. As a bonus, it's also a demonstration of Hazelcasts auto discovery mechanisms for client connections and also for cluster formation within Amazon EC2.

##In a nutshell
I use 3 main tools on my desktop, Vagrant allows me to script virtual machines and then using Chef I install the required software on these machines.  VirtualBox gives me the virtualisation platform on machine that Vagrant uses.  When I want to expand the Hazelcast cluster on more powerful hardware I simply tell Vagrant to use Amazon EC2 rather than VirtualBox.

### Vagrant

[Vagrant](https://www.vagrantup.com/) is a wonderful command line tool that lets you start up virtual machines either on your own desktop (using VirtualBox) or via another provider such as Amazon EC2.  Vagrant also takes care of running provisioner's such as Chef.  It simplifies mundane tasks such as connecting to the machines once they are up.

### Virtual Box

Is the virtualisation platform I use on my Macbook.  It's similar to VMWare, but it's free :-)

### Chef

Chef is used to install software on the newly created virtual machine.  The instructions to do this are created in files called recipes.  Take a tour around the [Chef getting started pages](https://learn.chef.io/)
...
##Where's the source code at?
The source code that accompanies this post is (or will be shortly) part of the official Hazelcast samples project.  

For now you can clone it from my personal github repository here...

[https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration](https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration)

You'll be looking for the code under....

[hazelcast-integration/amazon-ec2](https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration/hazelcast-integration/amazon-ec2)

The [README.md](https://github.com/dbrimley/hazelcast-code-samples/blob/amazon-ec2-integration/hazelcast-integration/amazon-ec2/README.md) there is pretty exhaustive and covers a lot of the same ground I'll delve into during this post.
...
Take a look at the [VagrantFile](https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration/hazelcast-integration/amazon-ec2/src/main/vagrant/Vagrantfile) within `/src/main/vagrant`
...
Lets take another look at the [VagrantFile](https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration/hazelcast-integration/amazon-ec2/src/main/vagrant/Vagrantfile) within `/src/main/vagrant`
...
Right now the Chef recipe just starts one Hazelcast JVM per Virtual Machine, you might want to experiment adding more JVMs per machine.  The simplest way to do this would be to copy/past the service line in the default.rb recipe.
...
Take a look at the Java [Client.java](https://github.com/dbrimley/hazelcast-code-samples/tree/amazon-ec2-integration/hazelcast-integration/amazon-ec2/src/main/java/com/hazelcast/samples/amazon/ec2/client/Client.java) within `/src/main/java/com/hazelcast/samples/amazon/ec2` 
